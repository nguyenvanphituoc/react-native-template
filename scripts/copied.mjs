...
async function getPaths(from, regx, paths = []) {
  try {
    const pathStr = (await fs.promises.readdir(from))
      .map((item) => path.join(from, item))
      .map((item) => (regx.test(item) ? false : item))
      .filter(Boolean);

    await Promise.all(
      pathStr.map(async (item) => {
...
async function copyDir(from, to, options = {}) {
  try {
    ...
    const pathArr = await getPaths(
      from,
      new RegExp("(/node_modules|/ios/Pods|Podfile.lock)")
    );
    await Promise.all(
      pathArr.map(async (item) => {
        ...
        if (item.isDir) {
          ...
          let relativePath = toPath.replace(
...
copyDir(
  path.resolve(process.cwd(), "sample_project"),
  path.resolve(process.cwd(), "template/template")
);
